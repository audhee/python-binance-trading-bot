...
class BinanceFuturesClient:
    ...
    def sync_server_time(self):
        ...
        try:
            ...
            logger.info(f"Time synced. Offset: {self.time_offset}ms (Server time: {server_time})")
        except requests.RequestException as e:
            logger.warning(f"Could not synchronize server time (using local clock). Error: {e}")
            self.time_offset = 0
